...
with st.sidebar:
    ...
    # Placeholder functions (ensure these are correctly implemented in metrics.py)
    try:
        metrics = get_metrics_summary()
    except:
        metrics = {"total_summaries": 0, "avg_processing_time": 0, "total_cost_usd": 0, "avg_compression_ratio": 0}

    if metrics and metrics.get("total_summaries", 0) > 0:
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Total", metrics["total_summaries"])
            st.metric("Time", f"{metrics['avg_processing_time']:.2f}s")
        with col2:
...
with main_tab1:
    ...
    with col_input:
        ...
        if "Paste" in input_type:
            ...
            if uploaded_file:
                # Ensure extract_text_from_file is implemented to handle these types
                try:
    ...
    with col_settings:
        ...
        with st.container(border=True):
            ...
            # Ensure get_all_versions is correctly implemented in prompt_versions.py
            try:
                all_versions = get_all_versions()
            except:
                all_versions = {"v1_basic": "Basic", "v3_chain_of_thought": "CoT"}
                
            selected_prompts = st.multiselect(
                "Select strategies to compare:",
                options=list(all_versions.keys()),
                default=["v1_basic", "v3_chain_of_thought"],
                format_func=lambda x: get_version_name(x) if 'get_version_name' in globals() else x
            )
    ...
    if run_btn and user_input and client:
        ...
        if not selected_prompts:
            st.warning("Please select at least one prompt strategy to run.")
        else:
            results_cache = {}
            ...
            with st.status("Processing Summaries...", expanded=True) as status:
                ...
                for v_key in selected_prompts:
                    ...
                    try:
                        ...
                        # Calculate Metrics (Ensure all evaluation functions work)
                        ...
                        st.error(f"Error generating summary for {v_name}: {e}")
            ...
            if results_cache:
                ...
                for tab, (key, data) in zip(tabs, results_cache.items()):
                    with tab:
                        ...
                        with m_col3:
                            # Safely access readability score
                            read_score = data['readability'].get('readability_score', 'N/A')
                            st.metric("Readability Score", f"{read_score}/100" if isinstance(read_score, (int, float)) else read_score)
                        with m_col4:
                        ...
                        with st.expander("🔍 View Technical Details"):
                            st.json({
                                ...
                                "Avg Sentence Length": data['readability'].get('avg_sentence_length', 'N/A'),
...
with main_tab3:
    ...
    try:
        ...
        st.error(f"Error loading evaluation history: {e}")
